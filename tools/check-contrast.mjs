...
const night = hex("#0b1530"), top = hex("#16245a");
const panel = over([22, 36, 82], 0.62, top); // 最も明るい上部に重ねた場合（厳しい側）
const quiet = over([107, 91, 214], 0.14, top);
const nowRow = over([217, 181, 106], 0.08, top);
const C = { moon: hex("#ede8dc"), sub: hex("#aab4cd"), gold: hex("#d9b56a"), goldSoft: hex("#f0d99b"), shu: hex("#e8705e"), err: hex("#ff9b8f"), field: hex("#8391b8"), focus: hex("#9cc3ff") };
const rows = [
  ...
  ["主ボタン文字(夜)/金", night, C.gold, 4.5], ["今月タグ文字/朱", hex("#060c22"), C.shu, 4.5], ["エラー/上部の紺", C.err, top, 4.5],
