...
for(const [key,f] of Object.entries(FOCUS)) {
  ...
  for(const star of TEN_GODS){
    ...
    assert.ok(got.theme && got.action && got.conclusion && got.example);
...
for(const key of ['love','relations','money','self']) {
  ...
  assert.ok(results.every(g=>!/(今の仕事|職場|本業|成果物|仕様)/.test(g.action+g.conclusion+g.description+g.example)));
}
